...
public class SudoSuite extends JavaPlugin
{
	...
	private String pVersion, pName, noPermColor, notifyColor, alertColor;
	...
	public void onEnable()
	{
		...
		else
		{
			...
			if(getConfig().getBoolean("subplugins.chat"))
			{
				...
				getCommand("shout").setExecutor(commandListener);
				getCommand("who").setExecutor(commandListener);
				getCommand("colors").setExecutor(commandListener);
				getCommand("list").setExecutor(commandListener);
			...
			{
				getCommand("slap").setExecutor(commandListener);
			}	
		}
	}
	...
	public void loadConfig()
	{
		...
		noPermColor = getConfig().getString("sudosuite.nopermissioncolor");
		notifyColor = getConfig().getString("sudosuite.notifycolor");
		alertColor = getConfig().getString("sudosuite.alertcolor");
	}
	...
	{
		tellPlayer("~" + noPermColor + getConfig().getString("sudosuite.nopermplayerformat").
	...
	public void alertPlayer(String message, Player player)
	{
		tellPlayer("§" + alertColor + message, player);
	}
	
	public void notifyPlayer(String message, Player player)
	{
		tellPlayer("§" + notifyColor + message, player);
	}
